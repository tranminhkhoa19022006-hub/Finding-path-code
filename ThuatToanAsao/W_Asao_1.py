...
def weighted_a_star(draw, grid, start, end): #Weighted A* 
    """
    Weighted A* với trọng số thích nghi theo khoảng cách từ start.
    f(n) = g(n) + (1 + g(n)/D) * h(n)
    """
    D = h(start.get_pos(), end.get_pos())  # Ước lượng khoảng cách từ start đến end
    ...
    g_score[start] = 0
    f_score[start] = g_score[start] + (1 + g_score[start] / D) * h(start.get_pos(), end.get_pos())
    open_set_hash = {start}
