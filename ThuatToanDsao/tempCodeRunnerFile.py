...
def run_robot(win, grid, rows, width, path, speed=0.5, start=None, end=None):
    ...
    while current != end and path:
        ...
        # đánh dấu ô đã đi qua thành màu tím
        ...
        clock.tick(int(1/speed))  # kiểm soát tốc độ bằng FPS
        ...
        if pygame.mouse.get_pressed()[0]:
            ...
            if new_spot != start and new_spot != end:
                ...
                # ✅ truyền robot=current để giữ màu đỏ
                new_path = d_star(lambda: draw(win, grid, rows, width), grid, current, end, robot=current)
                if new_path:
                    path = new_path
                    if current != start and current != end:
                        current.make_robot()

        # Chuột phải: cũng đặt barrier động
...
def d_star(draw, grid, start, end, robot=None):
    ...
    while current in b and b[current] is not None and current != end:
        current = b[current]
        # ✅ không vẽ đè lên ô robot
        if current != start and current != end and current != robot:
            current.make_path()
            path.append(current)
