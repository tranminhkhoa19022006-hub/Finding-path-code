...
def main(win, width):
    ...
    path_found = False   # trạng thái: đã tìm được đường đi chưa
    ...
    while run:
        ...
        for event in pygame.event.get():
            ...
            # Chuột trái: đặt start, end hoặc barrier
            if pygame.mouse.get_pressed()[0]:
                ...
                if not start and spot != end:
                    ...
                    spot.make_barrier()
                    # Nếu đã có đường đi rồi thì cập nhật lại ngay
                    if start and end and path_found:
                        for r in grid:
                            for s in r:
            ...
            if event.type == pygame.KEYDOWN:
                # SPACE: chạy D* lần đầu
                if event.key == pygame.K_SPACE and start and end:
                    for r in grid:
                        for s in r:
                            s.update_neighbors(grid)
                    d_star(lambda: draw(win, grid, ROWS, width), grid, start, end)
                    path_found = True   # đánh dấu đã tìm được đường đi

                # R: clear toàn bộ lưới
                if event.key == pygame.K_r:
                    start = None
                    end = None
                    grid = make_grid(ROWS, width)
                    path_found = False   # reset trạng thái
